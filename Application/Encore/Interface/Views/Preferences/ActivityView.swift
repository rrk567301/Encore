    ...
    internal var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            if let _ = self.bridge.applicationBundleIdentifier {
                Text(String(format: String(localized: "PREFERENCES_TEXT_LISTENING_TO"), "Apple Music"))
                    .font(.custom("Noto Sans", size: 12))
            } else {
                Text(String(format: String(localized: "PREFERENCES_TEXT_WELCOME"), Bundle.main.bundleName ?? ""))
                    .font(.custom("Noto Sans", size: 12))
            }
    ...
    internal var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            if self.isActive {
                ...
                TextView(text: String(localized: "PREFERENCES_TEXT_MAINTAINER"), size: 12, isBold: false)
                TextView(text: String(format: String(localized: "PREFERENCES_VERSION"), Bundle.main.bundleVersion ?? ""), size: 12, isBold: false)
                    .padding(.bottom, 4)

                HStack(alignment: .center, spacing: 0) {
                    TimestampView(timestamp: String(localized: "PREFERENCES_TEXT_COPYRIGHT"))
