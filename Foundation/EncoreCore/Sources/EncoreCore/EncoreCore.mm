...
- (void)connect {
    ...
    [[Logger sharedInstance] logMessage:@"[Core] Establishing a connection to the Discord client" forType:LogTypeInfo];
...
- (void)disconnect {
    ...
    [[Logger sharedInstance] logMessage:@"[Core] Shutting down the connection to the Discord client" forType:LogTypeInfo];
...
- (void)nowPlayingInfoChanged:(NSNotification* _Nonnull const)notification {
    [[Logger sharedInstance] logMessage:@"[Core] Received new now playing info" forType:LogTypeInfo];
...
- (void)updateRichPresence {
    [[Logger sharedInstance] logMessage:@"[Core] Updating the rich presence" forType:LogTypeInfo];
